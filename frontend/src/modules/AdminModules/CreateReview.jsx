...
export default class CreateReview extends Component {
  ...
  retrievePosts() {
    axios.get(`http://localhost:5000/checkreviewno`).then((res) => {
      if (res.data.success) {
        ...
        // Check if there are any reviews and if so, increment the execid_review by 1
        if (res.data.execid_review && res.data.execid_review.length > 0) {
  ...
  onSubmit = async (e) => {
    ...
    await axios.get(`http://localhost:5000/checkreviewno`).then((res) => {
      if (
        res.data.success &&
        res.data.execid_review &&
        res.data.execid_review.length > 0
      ) {
        this.setState({
          review: res.data.execid_review,
        });
    ...
    if (
      ...
      const data = {
        ...
        report: report,
        reportPDF: "https://example.com/report.pdfreportPDF",
        points: points,
        feedback: feedback,
        status: status,
      };
  ...
  render() {
    return (
      <div className="col-md-8 mt-4 mx-auto">
        <h1 className="h3 mb-3 font-weight-normal">Create New Review Test</h1> {/*remove the Test */}
